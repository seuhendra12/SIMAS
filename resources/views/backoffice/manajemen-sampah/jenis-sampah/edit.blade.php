...
@section('title','Ubah Data Jenis Sampah')
@section('container')
      ...
      <div id="kt_app_toolbar_container" class="app-container container-fluid d-flex flex-stack">
        <div class="page-title d-flex flex-column justify-content-center flex-wrap me-3">
          ...
          <ul class="breadcrumb breadcrumb-separatorless fw-semibold fs-7">
            ...
            <li class="breadcrumb-item text-muted">Ubah Data Jenis Sampah</li>
          </ul>
        </div>
      </div>
    </div>
            ...
            <div class="card card-flush h-md-100">
              <div class="card-body d-flex flex-column justify-content-between bgi-no-repeat bgi-size-cover bgi-position-x-center pb-0">
                ...
                <div>
                  ...
                  @endif
                  <form class="form" action="/jenis-sampah/{{$jenisSampah->id}}" method="POST">
                    @csrf
                    @method('PATCH')
                    <div class="d-flex flex-column mb-3 fv-row">
                      ...
                      <select class="form-select rounded-0" data-control="select2" data-hide-search="true" data-placeholder="Pilih Kategori Sampah" name="kategori_sampah" id="kategoriSampah">
                        ...
                        <option value="{{$kategoriSampah->id}}" {{ $jenisSampah->kategori_sampah_id == $kategoriSampah->id ? 'selected' : '' }}>
                          {{$kategoriSampah->name}}
                        </option>
                        @endforeach
                      </select>
                    ...
                    <div class="d-flex flex-column mb-3 fv-row">
                      ...
                      <input type="text" class="form-control rounded-0" placeholder="Masukkan Jenis Sampah" name="name" value="{{old('name',$jenisSampah->name)}}" />
                    </div>
                    <div class="d-flex flex-column mb-3 fv-row">
                      <label class="d-flex align-items-center fs-6 fw-semibold mb-2">
                        <span>Point Per Kilogram</span>
                      </label>
                      <input type="text" class="form-control rounded-0" placeholder="Masukkan jumlah point per kilogram" name="point_perkg" value="{{old('point_perkg',$jenisSampah->point_perkg)}}" />
                    </div>
                    <div class="d-flex flex-column mb-8 fv-row">
                      <label class="d-flex align-items-center fs-6 fw-semibold mb-2">
                        <span>Harga Jual Per Kilogram</span>
                      </label>
                      <input type="text" class="form-control rounded-0" placeholder="Masukkan Jumlah Harga Jual per Kilogram" name="harga_per_kg" value="{{old('harga_per_kg',$jenisSampah->harga_per_kg)}}"/>
                    </div>
                    <div class="mb-10">
