          ...
          <div class="col-xxl-6">
            <div class="card card-flush h-md-100">
              <div class="card-body d-flex flex-column justify-content-between bgi-no-repeat bgi-size-cover bgi-position-x-center pb-0">
                <div class="row">
                  <div class="col-8">
                    <div class="page-title d-flex flex-column justify-content-center flex-wrap me-3">
                      ...
                      <ul class="breadcrumb breadcrumb-separatorless fw-semibold fs-7 my-0 pt-1">
                        <li class="breadcrumb-item text-muted">
                          <p class="text-muted text-hover-primary">Data Kategori Sampah Pada Sistem Informasi Manajemen Sampah (SIMAS) Kelurahan Koto Luar</p>
                        </li>
                      </ul>
                    </div>
                  </div>
                  <div class="col-4">
                    <div class="d-grid d-md-flex justify-content-md-end">
                      <a href="kategori-sampah/create" class="btn btn-sm fw-bold btn-primary rounded-0 fw-bold">
                  ...
                  <div class="mt-3">
                    <div class="row">
                      ...
                      <div class="col-5 d-grid d-md-flex justify-content-md-end mb-3">
                        <form class="form" action="/kategori-sampah">
                    ...
                    <div class="table-container">
                      <table class="table table-bordered table-striped text-center">
                        ...
                        <tbody>
                          @forelse ($kategoriSampahs as $kategoriSampah)
                          <tr>
                            <td class="align-top">{{$loop->iteration}}</td>
                            <td class="align-top">{{$kategoriSampah->name}}</td>
                            <td class="align-middle">{{$kategoriSampah->deskripsi}}</td>
                            <td>
                              <a href="kategori-sampah/{{$kategoriSampah->id}}/edit" class="btn btn-yellow btn-sm button-action rounded-0 fw-bold" title="Edit" data-bs-toggle="tooltip">
                                <span>
                              ...
                              <a href="#" class="btn btn-red btn-sm rounded-0 fw-bold" data-bs-toggle="modal" data-bs-target="#confirm-delete-modal" data-kategori-sampah-id="{{ $kategoriSampah->id }}">
                                <span>
                ...
                <div class="mb-5">
                  {{$kategoriSampahs->appends(['perPage' => $perPage])->links('pagination::bootstrap-5')}}
                </div>
              </div>
            </div>
          </div>
      ...
      <div class="modal-dialog" role="document">
        <div class="modal-content rounded-0 fw-bold">
          <div class="modal-header">
            <h5 class="modal-title" id="confirm-delete-modal-label">Hapus Data Kategori Sampah</h5>
          ...
          <div class="modal-body">Apakah kamu yakin ingin menghapus kategori sampah ini?</div>
          <div class="modal-footer">
            <button type="button" class="btn btn-secondary rounded-0 fw-bold" data-bs-dismiss="modal">Batal</button>
            <form action="{{ url('kategori-sampah', '__kategoriSampah_id') }}" method="POST" style="display: inline-block;">
              @csrf
              @method('DELETE')
    ...
    <script>
      $(document).ready(function() {
        $("#confirm-delete-modal").on("show.bs.modal", function(e) {
          var kategoriSampahId = $(e.relatedTarget).data("kategori-sampah-id");
          console.log(kategoriSampahId);
          $(this)
            .find("form")
            .attr("action", function(index, value) {
              return value.replace("__kategoriSampah_id", kategoriSampahId);
            });
        });
      });
    </script>
